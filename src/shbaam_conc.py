...
def copy_global_attributes(output_netcdf4, input_netcdf4):
    ...
    for attribute_name in input_netcdf4.ncattrs():
        output_netcdf4.setncattr(attribute_name,                              \
        input_netcdf4.getncattr(attribute_name))
...
def copy_variables(output_netcdf4, input_netcdf4, input_files):
    ...
    for variable_name, variable in input_netcdf4.variables.iteritems():
        output_variable = output_netcdf4.createVariable(variable_name,        \
               variable.datatype, variable.dimensions)
        for attribute_name in variable.ncattrs():
           output_variable.setncattr(attribute_name,                          \
           variable.getncattr(attribute_name))
    ...
    for i in range(len(input_files)):
        ...
        output_netcdf4.variables['Canint'][i] =                               \
                                           input_file.variables['Canint'][0]
 
        start_time_strings.append(input_file.variables['time'].units)

        input_file.close()

    date_objects = [netCDF4.num2date(0, start_time_string,                    \
     input_netcdf4.variables['time'].calendar) for start_time_string          \
                in start_time_strings]
    for i in range(len(input_files)):
        output_netcdf4.variables['time'][i] = netCDF4.date2num(               \
                   date_objects[i], start_time_strings[0],                    \
                   input_netcdf4.variables['time'].calendar)
